...
def read_all_cryptos(spark, use_s3=False):
    ...
    - Usado apenas para verificação de arquivo vazio

    CORREÇÃO: count() removido após unionByName.
    O total de registros é informado apenas depois do persist(),
    evitando executar o plano de leitura duas vezes.
    """
    print("\n" + "="*60)
    ...
    # Combina todos os DataFrames — count() removido aqui intencionalmente:
    # contar antes do persist() forçaria a execução do plano de leitura
    # duas vezes. O total será exibido no relatório final.
    df_all = dfs[0]
    for df in dfs[1:]:
...
def transform_data(df):
    ...
    CORREÇÃO: count() após filtro removido.
    Chamar count() aqui dispararia uma Action antes do persist(),
    forçando o Spark a executar o plano completo duas vezes.
    O volume de dados será reportado no relatório final.
    """
    print("\n" + "="*60)
...
def gerar_relatorio_final(df):
    ...
    NOTA: o count() aqui é intencional — é a única Action de contagem
    do pipeline, executada após o persist() e após todas as análises.
    """
    print("\n" + "="*60)
...
def salvar_parquet(df, use_s3=False):
    ...
    CORREÇÃO: count() removido do print final.
    Contar após o write releria os dados já salvos desnecessariamente.
    O total é reportado no relatório final (gerar_relatorio_final),
    que é chamado antes desta função.
    """
    if use_s3:
...
def main():
    ...
    - persist() no df_transformed (reusado em 5 análises)
    ...
    Ordem intencional das operações:
    1. Leitura + transformação (lazy — nada executa ainda)
    2. persist() — materializa o DataFrame uma única vez
    3. Análises — todas reutilizam o cache (sem releitura)
    4. Relatório final — único count() intencional do pipeline
    5. Escrita em Parquet
    6. unpersist() — libera memória
    """
    use_s3 = "--s3" in sys.argv
    spark = create_spark_session(use_s3=use_s3)

    try:
        # Pipeline ETL — lazy até o persist()
        df_raw = read_all_cryptos(spark, use_s3)
        df_transformed = transform_data(df_raw)

        # OTIMIZAÇÃO: persist com controle de nível de storage
        # MEMORY_AND_DISK evita OOM em máquinas com pouca RAM
        df_transformed.persist(StorageLevel.MEMORY_AND_DISK)
        print("📦 DataFrame materializado e cacheado em MEMORY_AND_DISK")

        # Análises — todas reutilizam o cache sem releitura
        ...
        # Relatório final — count() único e intencional
